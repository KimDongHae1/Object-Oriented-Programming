...
public class MiddleBus extends Vehicle {

    private int AdditionalCost = 0;
    private int BasicCost = 3000;
    public MiddleBus( int VehicleNumber,Time time)
    {
    ...
    public double GetAdditionalCost()
    {
        ...
        {
            AdditionalCost =  this.getTime().getDay() * 72000 + (this.getTime().getHour()-1) * 3000;
            return AdditionalCost;
        }
        else if (this.getTime().getMinute()/30 == 0 && this.getTime().getMinute() != 0){
            AdditionalCost =  this.getTime().getDay() * 72000 + (this.getTime().getHour()-1) * 3000 + 1500;
            return AdditionalCost;
        }
        else if(this.getTime().getMinute()/30 == 1){
            AdditionalCost =  this.getTime().getDay() * 72000 + this.getTime().getHour() * 3000 + 3000;
            return AdditionalCost;
        }
        return 0;
    }
    ...
    {
        String string = "차량 종류 : 중형 버스" + ", 차량번호 : " + this.getVehicleNumber() + ", 입차 시간 : " + this.getTime().getYear()
